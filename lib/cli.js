...
function createFile(pathToRead, pathToWrite, answr) {
	...
	if (path.basename(pathToRead)[0] === ".") {
		pathToReadFormat = path.join(path.dirname(pathToRead), path.basename(pathToRead).replace(".", "_"));
	}

	const rs = typeof pathToReadFormat === "string" ? fs.createReadStream(pathToReadFormat) : fs.createReadStream(pathToRead);
	...
	.once("open", () => {
		...
		.on("open", async () => {
			...
			if (contentToReplace !== null) {
				const contentToInsert = contentToReplace.map(cd => answr[cd.match(/{{(.*?)}}/)[1]]);
				replaceInfile({
					files: ws.path,
