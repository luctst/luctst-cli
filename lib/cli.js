	...
	if (command[0] === "start") {
		...
		const filesToModify = [
			...
			},
			{
				...
				path: `${process.cwd()}`,
				toReplace: ["{{name}}","{{description}}","{{mail}}",/{{githubUser}}/g,/{{projectName}}/g],
				toInsert: [answers.name,answers.description,answers.mail,answers.githubUser,answers.projectName]
			}
		];

		copydir(path.join(process.cwd(), "lib","template"),`${process.cwd()}/test/`, {
				utimes: true,
				mode: true,
				cover: true,
			},
			function (err) {
				...
				filesToModify.map(function (item) {
					replaceInfile.sync({
