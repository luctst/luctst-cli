...
/**
 * Create a file
 * @param {Array} directory Directory where templates files lives.
 * @param {String} pathToRead The name of the file to be read.
 * @param {String} pathToWrite The name of the file to be written.
 */
function createFile(directory, pathToRead, pathToWrite) {
	

	const rs = fs.createReadStream(pathToRead);
}
...
async function createTree (opts = {}) {
	...
	const dirToRead = await fs.promises.readdir(pathToTake);
			...
			file.files.forEach(async subFile => {
				if (typeof subFile === "object") {
					...
					subFile.files.forEach(async innerFile => {
						await fs.promises.writeFile(path.join(pathToCreate, file.name, subFile.name, innerFile));
					})
				}

				await fs.promises.writeFile(path.join(pathToCreate, file.name, subFile));
			})
		}

		createFile(
			dirToRead,
			path.join(pathToTake, file)
		);
	});
}
